...
class Bot {
  constructor(options) {
    ...
    this.removeNicknameUsers = options.removeNicknameUsers;
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      // Add bold formatting:
      let fullMessage;
      if (this.removeNicknameUsers.includes(author)) {
        fullMessage = withMentions;
      } else {
        fullMessage = `**<${author}>** ${withMentions}`;
      }

      logger.debug('Sending message to Discord', fullMessage, channel, '->', discordChannelName);
      discordChannel.sendMessage(fullMessage);
    }
  }
}
